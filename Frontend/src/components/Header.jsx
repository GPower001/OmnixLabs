...
//               <li key={index} className={links.name === "Generate Invoice" ? "my-button" : ""}>
//                 {links.name === "Generate Invoice" ? (
  ...
  return (
    ...
    <div className="flex justify-between items-center px-6 md:px-10 h-16 md:h-20 transition-all duration-500">
      ...
      >
        <ul className="flex flex-col gap-6 px-6">
          {NavLinks.map((links, index) => (
            <li key={index} className={links.name === "Generate Invoice" ? "my-button w-max" : ""}>
              {links.name === "Generate Invoice" ? (
                <a 
                  href={links.href} 
                  target="_blank" 
